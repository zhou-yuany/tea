  ...
  <div class="app-container">
    ...
    >
      ...
      <el-form-item
        ...
        >
          <el-select
            v-model="couponInfo.goodRangeList"
            multiple
            placeholder="请选择"
            style="width: 100%"
          >
        ...
        >
          ...
          <span>{{ couponInfo.type == 1 ? "满减" : "新用户" }}</span>
        </el-form-item>
        ...
        >
          <!-- <el-date-picker
            ...
            value-format="yyyy-MM-dd HH:mm"
          ></el-date-picker> -->
          <span>{{ couponInfo.startTime }}</span>
        </el-form-item>
        <el-form-item
          label="活动结束时间："
          prop="endTime"
    ...
    >
      ...
      <el-table-column prop="goodsName" label="活动商品" align="center">
      </el-table-column>
      ...
      <el-table-column prop="limits" label="金额限制" align="center">
      </el-table-column>
...
export default {
  ...
  methods: {
    getGoods() {
      ...
      couponApi.getGoods(shopId).then((response) => {
    ...
    getList(page = 1) {
      ...
      let shopId = JSON.parse(localStorage.getItem("adminInfo")).id;
      couponApi
        .getList(this.page, this.limit, shopId, this.couponQuery)
        .then((response) => {
    ...
    openEditCoupon(data) {
      ...
      couponApi.getCouponInfo(data.id).then((response) => {
        ...
        if (this.couponInfo.goodRangeList == null) {
          this.couponInfo.goodRangeList = [];
        }
      });
    },
    // 修改信息
