<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="订单来源" width="width" align="center">
        <template slot-scope="scope">
          {{
      ...
      <el-table-column label="订单类型" width="width" align="center">
        <template slot-scope="scope">
          {{ scope.row.equipmentType == 1 ? "外卖单" : "自助单" }}
        </template>
      </el-table-column>
    ...
    <el-dialog :visible.sync="dialogInfo" title="订单详情">
      <el-form :model="ordersInfo" label-width="60px">
        ...
        <el-form-item label="订单来源：" style="width: 45%" label-width="120px">
          <span>{{
            ordersInfo.platformType == 1
              ? "茶饮小程序"
              : ordersInfo.platformType == 2
              ? "饿了么"
              : ordersInfo.platformType == 3
              ? "美团"
              : "竖屏"
          }}</span>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    getList(page = 1) {
      ...
      orderApi
        .ordersList(this.page, this.limit, shopId, this.orderQuery)
        .then((response) => {
